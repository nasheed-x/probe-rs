...
use wasm_bindgen_futures::{JsFuture, js_sys::Array, spawn_local, wasm_bindgen::JsCast};
use web_sys::{
    UsbControlTransferParameters, UsbDevice, UsbInTransferResult, UsbOutTransferResult,
    UsbTransferStatus,
    js_sys::{DataView, Object, Uint8Array},
};

fn uint8_array_from_data_view(data: &DataView) -> Uint8Array {
    // WebUSB returns a DataView whose bytes may start part-way through its
    // backing ArrayBuffer. Constructing a Uint8Array from only the buffer
    // silently discards that range and can expose stale bytes from an earlier
    // transfer as the CMSIS-DAP response.
    Uint8Array::new_with_byte_offset_and_length(
        &data.buffer(),
        data.byte_offset().min(u32::MAX as usize) as u32,
        data.byte_length().min(u32::MAX as usize) as u32,
    )
}
...
use crate::{
    ...
    descriptors::{
        ConfigurationDescriptor, DESCRIPTOR_TYPE_CONFIGURATION, DeviceDescriptor,
        EndpointDescriptor,
    },
    maybe_future::future::ActualFuture,
    transfer::{
        Buffer, Completion, ControlIn, ControlOut, Direction, TransferError,
        internal::{Idle, Notify, Pending, notify_completion, take_completed_from_queue},
    },
};

use super::{
    TransferData, js_value_to_io_error, js_value_to_transfer_error,
    webusb_status_to_nusb_transfer_error,
};
...
pub async fn get_descriptor(
    ...
    Ok(uint8_array_from_data_view(&res.data().expect("a data buffer")).to_vec())
}
...
pub async fn extract_string(device: &UsbDevice, id: u16) -> Result<String, Error> {
    ...
    let res = JsFuture::from(device.control_transfer_in(&setup, 255))
        ...
        .map_err(js_value_to_io_error)?;
    let res: UsbInTransferResult = JsCast::unchecked_from_js(res);
    let mut data = uint8_array_from_data_view(&res.data().expect("a data buffer")).to_vec();
...
impl WebusbInterface {
    ...
    pub fn control_in(
        ...
        ActualFuture::new(async move {
            ...
            let res: UsbInTransferResult = JsCast::unchecked_from_js(res);
            let array = uint8_array_from_data_view(&res.data().expect("a data buffer"));
...
impl WebusbEndpoint {
    ...
    pub(crate) fn submit(&mut self, buffer: Buffer) {
        ...
        // TransferData owns the allocation while the browser promise is in
        // flight. IN buffers have an initialized length of zero, so their
        // requested length must be copied explicitly instead of using len().
        let transfer_len = match dir {
            Direction::Out => buffer.len,
            Direction::In => {
                let packet_size = self.max_packet_size as u32;
                if buffer.requested_len > packet_size {
                    buffer.requested_len - (buffer.requested_len % packet_size)
                } else {
        ...
        transfer.requested_len = transfer_len;
            ...
            match dir {
                Direction::Out => {
                    ...
                    let Ok(result) = result else {
                        unsafe {
                            (*ptr).status = UsbTransferStatus::Stall;
                            (*ptr).actual_len = 0;
                ...
                Direction::In => {
                    ...
                    let received_data = uint8_array_from_data_view(
                        &transfer_result.data().expect("a data buffer is present"),
                    );
                    let received_len = received_data.length().min(buffer.capacity) as usize;
                    let target =
                        unsafe { std::slice::from_raw_parts_mut(buffer.ptr, received_len) };
                    received_data
                        .subarray(0, received_len as u32)
                        .copy_to(target);

                    unsafe {
                        (*ptr).status = transfer_result.status();
                        (*ptr).actual_len = received_len as u32;
                        notify_completion::<TransferData>(ptr)
                    }
                }
            }
